...
public class KissClientEvent {

    public static void registerClient() {
        ClientPlayNetworking.registerGlobalReceiver(KissServerEvent.KISS_PARTICLE_PACKET_ID, (client, handler, buf, sender) -> {
            BlockPos pos = buf.readBlockPos();
            client.execute(() -> spawnHeartParticles(client, pos));
        });

        ClientPlayNetworking.registerGlobalReceiver(KissServerEvent.KISS_SOUND_PACKET_ID, (client, handler, buf, sender) -> {
            BlockPos pos = buf.readBlockPos();
            ...
            client.execute(() -> {
                if (client.world != null && client.player != null) {
                    ...
                    float finalVolume = serverComputedVolume * config.kissVolume;

                    if (finalVolume > 0.01f) {
                        client.world.playSound(client.player, pos, KissServerEvent.KISS_SOUND_EVENT, net.minecraft.sound.SoundCategory.PLAYERS, finalVolume, 1.0f);
                    }
                }
            });
        });
